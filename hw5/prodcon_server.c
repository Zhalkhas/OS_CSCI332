...
void* conn_handler( void* arg )
{
	...
	if ( strcmp( "PRODUCE\r\n", charbuf ) == 0 ) {
		...
		prods += free;
		pthread_mutex_unlock( &client_mutex );
		free ? producer( ssock ) : exit_client( 0 );
	} else if ( strcmp( "CONSUME\r\n", charbuf ) == 0 ) {
		pthread_mutex_lock( &client_mutex );
		bool free = is_free( CONSUMER );
		cons += free;
		pthread_mutex_unlock( &client_mutex );
		free ? consumer( ssock ) : exit_client( 0 );
	} else {
		exit_client( 0 );
		close( ssock );
		pthread_exit( NULL );
...
int main( int argc, char** argv )
{
	...
	for ( ;; ) {
		...
		*ssock     = accept( msock, ( struct sockaddr* ) &fsin, &alen );
		...
		free ? pthread_create( &thread, NULL, conn_handler, ( void* ) ssock ) : close( *ssock );
	}
}