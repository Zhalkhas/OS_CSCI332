...
void exit_thread( char msg[], int bytes, int len )
{
	char name[ 40 ];
	char str[ 40 ];
...
void* consumer( void* arg )
{
	...
	if ( cc != len ) {
		while ( cc != 0 && read_bytes < len ) {
			if ( cc < 0 ) {
				fprintf( stderr, "could not get str\n" );
				exit_thread( BYTE_ERROR, read_bytes, len );
			}
	...
	exit_thread( ( read_bytes != len ? BYTE_ERROR : SUCCESS ), read_bytes, len );
}
